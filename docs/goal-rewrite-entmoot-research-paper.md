...
4. `paper/entmoot-paper.pdf` and `paper/evolution.pdf` are the tracked PDFs
   to update; both `website/static/papers/` copies are generated from them by
   `npm run sync-papers` and ignored, so leave those alone.
...
- Build the final paper and update the tracked PDF in `paper/`.
...
- Do not commit anything under `website/static/papers/`: `npm run
  sync-papers` copies both PDFs there from `paper/` on every website build,
  and both copies are ignored.
