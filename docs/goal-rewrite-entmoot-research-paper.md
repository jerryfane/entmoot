...
4. `paper/entmoot-paper.pdf` is the tracked PDF to update;
   `website/static/papers/entmoot-main.pdf` is generated by
   `npm run sync-papers` and ignored, so leave it alone.
