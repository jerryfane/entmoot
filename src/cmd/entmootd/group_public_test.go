...
func TestCmdGroupPublicDescriptorHelpStopsBeforeSetup(t *testing.T) {
	...
	if code != exitOK {
		t.Fatalf("descriptor help code=%d, want %d", code, exitOK)
	}
	if stdout != "" {
		t.Fatalf("descriptor help stdout=%q, want empty", stdout)
	}
	if !strings.Contains(stderr, "Usage of group public descriptor") {
		t.Fatalf("descriptor help stderr=%q, want usage", stderr)
	}
	if strings.Contains(stderr, "-group:") || strings.Contains(stderr, "identity") {
		t.Fatalf("descriptor help stderr=%q, command continued past help", stderr)
	}
}
...
func TestCmdGroupPublicPublishHelpStopsBeforeSetup(t *testing.T) {
	...
	if code != exitOK {
		t.Fatalf("publish help code=%d, want %d", code, exitOK)
	}
	if stdout != "" {
		t.Fatalf("publish help stdout=%q, want empty", stdout)
	}
	if !strings.Contains(stderr, "Usage of group public publish") {
		t.Fatalf("publish help stderr=%q, want usage", stderr)
	}
	if strings.Contains(stderr, "-group:") || strings.Contains(stderr, "-esp-url is required") || strings.Contains(stderr, "identity") {
		t.Fatalf("publish help stderr=%q, command continued past help", stderr)
	}
}
