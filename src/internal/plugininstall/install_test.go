...
func TestInstallCodexCommandOrder(t *testing.T) {
	home := filepath.Join(t.TempDir(), ".entmoot")
...
func TestInstallClaudeCommandOrder(t *testing.T) {
	...
	want := []fakeCall{
		...
		{command: "claude", args: []string{"plugin", "uninstall", "entmoot@entmoot-local", "--scope", "project", "--keep-data"}},
		{command: "claude", args: []string{"plugin", "install", "entmoot@entmoot-local", "--scope", "project"}},
	}
...
func TestInstallValidatesClaudeScope(t *testing.T) {
	_, err := Install(context.Background(), Options{
		Provider: pluginpack.ProviderClaude,
		Home:     t.TempDir(),
		Scope:    "team",
	...
	if err == nil || !strings.Contains(err.Error(), "unknown claude plugin scope") {
		t.Fatalf("Install() error = %v, want scope error", err)
	}
}

func TestMissingRuntimeManualCommandsUseShellSafeQuotes(t *testing.T) {
	home := filepath.Join(t.TempDir(), "entmoot '$HOME';next")
	result, err := Install(context.Background(), Options{
		Provider: pluginpack.ProviderClaude,
		Home:     home,
		Runner:   &fakeRunner{},
...
func (r *fakeRunner) Run(ctx context.Context, dir string, command string, args ...string) (subprocess.Result, error) {
	...
	if r.failUninstallNotFound && command == "claude" && len(args) >= 2 && args[0] == "plugin" && args[1] == "uninstall" {
		return subprocess.Result{Command: command, Args: args, Stderr: `Plugin "entmoot@entmoot-local" not found in installed plugins`}, errors.New("exit status 1")
	}
