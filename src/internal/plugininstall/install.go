...
func Install(ctx context.Context, opts Options) (Result, error) {
	...
	build, err := buildPackage(provider, opts.Home, packagePath, opts.Force, opts.Version)
	if err != nil {
		return Result{}, err
	}
	if _, err := buildPackage(provider, opts.Home, marketplacePackagePath, opts.Force, opts.Version); err != nil {
		return Result{}, err
	}
...
func buildPackage(provider pluginpack.Provider, home string, outDir string, force bool, version string) (pluginpack.BuildResult, error) {
	buildForce := force
	if !buildForce {
...
func marketplacePayload(provider pluginpack.Provider) any {
	...
	case pluginpack.ProviderCodex:
		return codexMarketplace{
			...
			Interface: marketplaceInterface{
				DisplayName: pluginpack.DisplayName + " Local",
			},
	...
	case pluginpack.ProviderClaude:
		return claudeMarketplace{
			...
			Name:        pluginpack.MarketplaceName,
			Description: "Local Entmoot plugin marketplace.",
			Owner:       claudeOwner{Name: pluginpack.DisplayName},
			Plugins: []claudeMarketplacePlugin{{
				Name:        pluginpack.PluginName,
				Description: pluginpack.Description,
				Author:      claudeOwner{Name: pluginpack.DisplayName},
				Category:    "productivity",
