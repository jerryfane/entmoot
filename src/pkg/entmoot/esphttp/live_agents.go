...
type LiveAgentConfig struct {
	...
	TopicFilters      []string         `json:"topic_filters,omitempty"`
	AllowedActions    []string         `json:"allowed_actions,omitempty"`
	MaxActionsPerScan int              `json:"max_actions_per_scan"`
	MaxActionBytes    int              `json:"max_action_bytes"`
...
type LiveAgentCursor struct {
	...
	LastSeenAtMS           int64               `json:"last_seen_at_ms"`
	LastSeenAuthorMemberID entmoot.MemberID    `json:"last_seen_author_member_id,omitempty"`
	LastSeenMessageID      entmoot.MessageID   `json:"last_seen_message_id"`
...
func ValidateLiveConfig(cfg LiveAgentConfig) error {
	...
	if cfg.MemberID == (entmoot.MemberID{}) {
		return errors.New("esphttp: live member id is required")
	}
...
func (s *MemoryStateStore) UpsertLiveAgentConfig(_ context.Context, cfg LiveAgentConfig) (LiveAgentConfig, error) {
	...
	if s.liveAgentConfigs[cfg.GroupID] == nil {
		s.liveAgentConfigs[cfg.GroupID] = make(map[entmoot.MemberID]LiveAgentConfig)
	}
	if cfg.UpdatedAtMS == 0 {
		cfg.UpdatedAtMS = s.nowMS()
	}
	s.liveAgentConfigs[cfg.GroupID][cfg.MemberID] = cloneLiveAgentConfig(cfg)
	return cloneLiveAgentConfig(cfg), nil
}

func (s *MemoryStateStore) GetLiveAgentConfig(_ context.Context, gid entmoot.GroupID, nodeID entmoot.MemberID) (LiveAgentConfig, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *MemoryStateStore) ListLiveAgentConfigsForMember(_ context.Context, memberID entmoot.MemberID) ([]LiveAgentConfig, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	out := make([]LiveAgentConfig, 0)
	for _, configs := range s.liveAgentConfigs {
		if cfg, ok := configs[memberID]; ok {
			out = append(out, cloneLiveAgentConfig(cfg))
		}
	}
...
func (s *MemoryStateStore) DeleteLiveAgentConfig(_ context.Context, gid entmoot.GroupID, nodeID entmoot.MemberID, updatedAtMS int64) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *MemoryStateStore) UpsertLiveAgentPresence(_ context.Context, p LiveAgentPresence) (LiveAgentPresence, error) {
	...
	if p.MemberID == (entmoot.MemberID{}) {
		return LiveAgentPresence{}, errors.New("esphttp: live presence member id is required")
	}
	if p.Mode == "" {
	...
	if s.liveAgentPresence[p.GroupID] == nil {
		s.liveAgentPresence[p.GroupID] = make(map[entmoot.MemberID]LiveAgentPresence)
	}
	s.liveAgentPresence[p.GroupID][p.MemberID] = cloneLiveAgentPresence(p)
...
func (s *MemoryStateStore) GetLiveAgentCursor(_ context.Context, gid entmoot.GroupID, nodeID entmoot.MemberID) (LiveAgentCursor, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *MemoryStateStore) UpsertLiveAgentCursor(_ context.Context, cursor LiveAgentCursor) (LiveAgentCursor, error) {
	...
	if cursor.MemberID == (entmoot.MemberID{}) {
		return LiveAgentCursor{}, errors.New("esphttp: live cursor member id is required")
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if s.liveAgentCursors[cursor.GroupID] == nil {
		s.liveAgentCursors[cursor.GroupID] = make(map[entmoot.MemberID]LiveAgentCursor)
	}
	s.liveAgentCursors[cursor.GroupID][cursor.MemberID] = cloneLiveAgentCursor(cursor)
...
INSERT INTO esp_live_agent_configs (group_id, member_id, enabled, mode, topic_filters, allowed_actions, max_actions_per_scan, max_action_bytes, updated_at_ms)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
ON CONFLICT(group_id, member_id) DO UPDATE SET
  ...
  updated_at_ms=excluded.updated_at_ms`,
		cfg.GroupID[:], cfg.MemberID[:], enabled, cfg.Mode, topics, actions, cfg.MaxActionsPerScan, cfg.MaxActionBytes, cfg.UpdatedAtMS)
	if err != nil {
...
func (s *SQLiteStateStore) GetLiveAgentConfig(ctx context.Context, gid entmoot.GroupID, memberID entmoot.MemberID) (LiveAgentConfig, bool, error) {
	row := s.db.QueryRowContext(ctx, `SELECT group_id, member_id, enabled, mode, topic_filters, allowed_actions, max_actions_per_scan, max_action_bytes, updated_at_ms FROM esp_live_agent_configs WHERE group_id = ? AND member_id = ?`, gid[:], memberID[:])
...
func (s *SQLiteStateStore) ListLiveAgentConfigs(ctx context.Context, gid entmoot.GroupID) ([]LiveAgentConfig, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT group_id, member_id, enabled, mode, topic_filters, allowed_actions, max_actions_per_scan, max_action_bytes, updated_at_ms FROM esp_live_agent_configs WHERE group_id = ? ORDER BY member_id`, gid[:])
	if err != nil {
...
func (s *SQLiteStateStore) ListLiveAgentConfigsForMember(ctx context.Context, memberID entmoot.MemberID) ([]LiveAgentConfig, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT group_id, member_id, enabled, mode, topic_filters, allowed_actions, max_actions_per_scan, max_action_bytes, updated_at_ms FROM esp_live_agent_configs WHERE member_id = ? ORDER BY group_id`, memberID[:])
	if err != nil {
		return nil, fmt.Errorf("esphttp: list live agent configs for member: %w", err)
	}
	defer rows.Close()
	out := []LiveAgentConfig{}
	for rows.Next() {
...
func (s *SQLiteStateStore) DeleteLiveAgentConfig(ctx context.Context, gid entmoot.GroupID, memberID entmoot.MemberID, updatedAtMS int64) error {
	if updatedAtMS == 0 {
		updatedAtMS = nowMS()
	}
	...
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, `DELETE FROM esp_live_agent_configs WHERE group_id = ? AND member_id = ?`, gid[:], memberID[:]); err != nil {
		return fmt.Errorf("esphttp: delete live agent config: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `UPDATE esp_live_agent_presence SET status = ?, updated_at_ms = ? WHERE group_id = ? AND member_id = ?`, LiveStatusOffline, updatedAtMS, gid[:], memberID[:]); err != nil {
...
func (s *SQLiteStateStore) UpsertLiveAgentPresence(ctx context.Context, p LiveAgentPresence) (LiveAgentPresence, error) {
	...
	_, err := s.db.ExecContext(ctx, `
INSERT INTO esp_live_agent_presence (group_id, member_id, status, mode, topic_filters, last_seen_at_ms, lease_until_ms, updated_at_ms)
VALUES (?, ?, ?, ?, ?, ?, ?, ?)
ON CONFLICT(group_id, member_id) DO UPDATE SET
  ...
  updated_at_ms=excluded.updated_at_ms`,
		p.GroupID[:], p.MemberID[:], p.Status, p.Mode, topics, p.LastSeenAtMS, p.LeaseUntilMS, p.UpdatedAtMS)
	if err != nil {
...
func (s *SQLiteStateStore) ListLiveAgentPresence(ctx context.Context, gid entmoot.GroupID) ([]LiveAgentPresence, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT group_id, member_id, status, mode, topic_filters, last_seen_at_ms, lease_until_ms, updated_at_ms FROM esp_live_agent_presence WHERE group_id = ? ORDER BY member_id`, gid[:])
	if err != nil {
...
func (s *SQLiteStateStore) GetLiveAgentCursor(ctx context.Context, gid entmoot.GroupID, memberID entmoot.MemberID) (LiveAgentCursor, bool, error) {
	row := s.db.QueryRowContext(ctx, `SELECT group_id, member_id, scan_floor_at_ms, last_seen_at_ms, last_seen_author_member_id, last_seen_message_id, seen_message_ids, updated_at_ms FROM esp_live_agent_cursors WHERE group_id = ? AND member_id = ?`, gid[:], memberID[:])
...
func (s *SQLiteStateStore) UpsertLiveAgentCursor(ctx context.Context, cursor LiveAgentCursor) (LiveAgentCursor, error) {
	...
	_, err = s.db.ExecContext(ctx, `
INSERT INTO esp_live_agent_cursors (group_id, member_id, scan_floor_at_ms, last_seen_at_ms, last_seen_author_member_id, last_seen_message_id, seen_message_ids, updated_at_ms)
VALUES (?, ?, ?, ?, ?, ?, ?, ?)
ON CONFLICT(group_id, member_id) DO UPDATE SET
  scan_floor_at_ms=excluded.scan_floor_at_ms,
  last_seen_at_ms=excluded.last_seen_at_ms,
  last_seen_author_member_id=excluded.last_seen_author_member_id,
  last_seen_message_id=excluded.last_seen_message_id,
  seen_message_ids=excluded.seen_message_ids,
  updated_at_ms=excluded.updated_at_ms`,
		cursor.GroupID[:], cursor.MemberID[:], cursor.ScanFloorAtMS, cursor.LastSeenAtMS, cursor.LastSeenAuthorMemberID[:], cursor.LastSeenMessageID[:], seenIDs, cursor.UpdatedAtMS)
	if err != nil {
...
func LiveAgentStatesByMember(configs []LiveAgentConfig, presences []LiveAgentPresence, nowMS int64) map[entmoot.MemberID]LiveAgentState {
	out := make(map[entmoot.MemberID]LiveAgentState, len(configs))
	for _, cfg := range configs {
		if !cfg.Enabled {
			continue
		}
		out[cfg.MemberID] = LiveAgentState{
			Enabled:           cfg.Enabled,
...
func scanLiveAgentConfig(row interface {
	...
	var groupBytes, memberBytes []byte
	var enabled int
	var topics, actions []byte
	if err := row.Scan(&groupBytes, &memberBytes, &enabled, &cfg.Mode, &topics, &actions, &cfg.MaxActionsPerScan, &cfg.MaxActionBytes, &cfg.UpdatedAtMS); err != nil {
		return LiveAgentConfig{}, err
	}
	if err := decodeGroupIDBytes(groupBytes, &cfg.GroupID); err != nil {
		return LiveAgentConfig{}, err
	}
	if len(memberBytes) != len(cfg.MemberID) {
		return LiveAgentConfig{}, errors.New("esphttp: invalid live config member id")
	}
	copy(cfg.MemberID[:], memberBytes)
...
func scanLiveAgentPresence(row interface {
	...
	var groupBytes, memberBytes []byte
	var topics []byte
	if err := row.Scan(&groupBytes, &memberBytes, &p.Status, &p.Mode, &topics, &p.LastSeenAtMS, &p.LeaseUntilMS, &p.UpdatedAtMS); err != nil {
		return LiveAgentPresence{}, err
	}
	if err := decodeGroupIDBytes(groupBytes, &p.GroupID); err != nil {
		return LiveAgentPresence{}, err
	}
	if len(memberBytes) != len(p.MemberID) {
		return LiveAgentPresence{}, errors.New("esphttp: invalid live presence member id")
	}
	copy(p.MemberID[:], memberBytes)
...
func scanLiveAgentCursor(row interface {
	...
	var groupBytes, memberBytes, authorBytes, msgID, seenIDs []byte
	if err := row.Scan(&groupBytes, &memberBytes, &cursor.ScanFloorAtMS, &cursor.LastSeenAtMS, &authorBytes, &msgID, &seenIDs, &cursor.UpdatedAtMS); err != nil {
		return LiveAgentCursor{}, err
	}
	if err := decodeGroupIDBytes(groupBytes, &cursor.GroupID); err != nil {
		return LiveAgentCursor{}, err
	}
	if len(memberBytes) != len(cursor.MemberID) {
		return LiveAgentCursor{}, errors.New("esphttp: invalid live cursor member id")
	}
	copy(cursor.MemberID[:], memberBytes)
	if len(authorBytes) == len(cursor.LastSeenAuthorMemberID) {
		copy(cursor.LastSeenAuthorMemberID[:], authorBytes)
	}
...
func sortLiveAgentConfigs(items []LiveAgentConfig) {
	sort.Slice(items, func(i, j int) bool {
		return bytes.Compare(items[i].MemberID[:], items[j].MemberID[:]) < 0
	})
}
