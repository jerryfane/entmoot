...
Entmoot sits between agent-network research, distributed messaging, and
peer-to-peer replication. The direct empirical peer is
\citet{calin2026emergent}: it establishes that a nontrivial Pilot agent
population exists and that its social structure is measurable even when
content remains encrypted. Entmoot contributes the group layer that such a
population lacks. It does not replace Pilot's identity or tunnel layer, and it
does not prescribe a planning or game-theoretic coordination model of the kind
studied in classical multi-agent systems
literature~\citep{wooldridge2009,shohamleyton2008,dorri2018}. Instead, it
provides the shared rooms, history, discovery, and policy surface that agents
can use before any higher-level coordination protocol begins.

Brokered pub/sub systems and federated chat systems solve adjacent problems
with different trust assumptions. MQTT and NATS provide efficient topic or
subject fanout~\citep{mqtt5,natsPubSub}, but they assume a broker or server
cluster that mediates delivery. Matrix provides federated room state and
client/server access~\citep{matrixSpec}; ActivityPub provides federated social
object delivery through inboxes and outboxes~\citep{activitypub}. Entmoot
borrows the user-facing shape of rooms, topics, and intermittent clients, but
the signed group state remains portable across ESPs. The ESP can index public
descriptors, serve mobile clients, cache cursors, and expose search, yet it is
not the source of membership truth.

Peer-to-peer replication systems are closer to Entmoot's failure model.
Secure Scuttlebutt~\citep{tarr2019ssb} is the nearest published analogue: a
signed append-only log per identity, disseminated by gossip and repaired by
anti-entropy. Entmoot keeps that signed-log discipline but changes the unit of
organization from global social feeds to trust-scoped moots with explicit
membership, invites, policy, and public discovery metadata. IPFS shares
content-addressing~\citep{benet2014ipfs}; Certificate Transparency supplies
Merkle-log verification vocabulary~\citep{laurie2013ct}; and Range-Based Set
Reconciliation supplies the repair pattern for divergent message
sets~\citep{meyer2023rbsr}. Plumtree and GossipSub motivate the split between
fast-path push, lazy announcements, and repair~\citep{leitao2007plumtree,gossipsubSpec,vyzovitis2020gossipsub}.
The main difference is scope: Entmoot gossips inside signed, Pilot-trusted
groups rather than across an open pub/sub mesh.

Finally, Entmoot is intentionally not a task orchestration system. Fleet and
task workflows can be built as opt-in application conventions on top of moots,
but the default protocol purpose is social chat and shared memory. That choice
keeps the core comparable to communication systems rather than schedulers:
membership, delivery, reconciliation, search, and public discovery are the
paper's system concerns; planning and task allocation are downstream uses.
