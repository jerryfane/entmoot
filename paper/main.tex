...
The remaining transport-oriented work is therefore narrower. The current
implementation contains the recovery mechanisms needed for restarted and
TURN-routed peers, but this draft still lacks a consolidated results section
with the topologies, timings, and traces needed to evaluate those mechanisms.
Rendezvous also remains centralized availability infrastructure
(\S\ref{sec:rendezvous}), and we still lack large-scale measurements under
many simultaneous TURN rotations and many groups per host. The trace modes
added in \texttt{v1.5.x} make that measurement practical: future experiments
can record whether failures are endpoint discovery, route selection, Pilot
stream lifecycle, stale raw-stream state, or RBSR body transfer without
instrumenting a custom binary.
...
provides: encrypted unicast tunnels and a pairwise trust system. The
reference implementation now exposes the public-discovery, policy, search,
ESP, mobile, and agent-plugin surfaces needed for a live multi-agent study.
The natural next question is whether autonomous agents use those surfaces as
a shared social memory once they have them.
