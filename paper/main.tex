...
\begin{tikzpicture}[
  ...
  stack/.style={box, minimum width=44mm, fill=gray!10},
  state/.style={box, minimum width=35mm, fill=gray!5},
  arrow/.style={-{Latex[length=1.8mm]}, thick},
  note/.style={font=\scriptsize, align=center},
]
\node[stack] (clients) at (0, 2.4) {CLI and local agents\\web and iOS clients};
\node[stack, fill=gray!18] (ent) at (0, 1.0) {\textbf{entmootd}\\signed group service};
\node[stack] (pilot) at (0, -0.4) {\textbf{Pilot daemon}\\identity, trust, NAT, streams};
\node[stack] (remote) at (0, -1.8) {remote Pilot daemons\\remote \texttt{entmootd} peers};

\node[state] (truth) at (5.1, 1.0) {signed moot state\\rosters, messages, policies};
\node[state] (esp) at (-5.1, 1.0) {ESP projection\\HTTP, sessions, cursors};

\draw[arrow] (clients) -- node[right, note] {control socket} (ent);
\draw[arrow] (clients.west) -- ++(-1.2,0) |- node[left, note, pos=0.25] {HTTP} (esp.north);
\draw[arrow] (esp) -- node[above, note] {sign requests\\history, search} (ent);
\draw[arrow] (ent) -- node[right, note] {Pilot IPC} (pilot);
\draw[arrow] (pilot) -- node[right, note] {encrypted streams} (remote);
\draw[arrow] (ent) -- node[above, note] {verify/project} (truth);
\draw[arrow, dashed] (truth) -- node[below, note] {portable truth} (ent);
\end{tikzpicture}
...
\node[signed] (roster) at (0, 1.8) {roster\\and roles};
\node[signed] (msgs) at (3.0, 1.8) {author-signed\\messages};
\node[signed] (policy) at (6.0, 1.8) {founder-signed\\policies};
\node[signed] (desc) at (1.5, 0.6) {public\\descriptors};
\node[signed] (profile) at (4.5, 0.6) {member\\profiles};

\node[local] (sessions) at (0, -1.0) {device\\sessions};
\node[local] (cursors) at (3.0, -1.0) {mailbox\\cursors};
\node[local] (search) at (6.0, -1.0) {search\\index};
\node[local] (dir) at (1.5, -2.2) {directory\\rows};
\node[local] (push) at (4.5, -2.2) {push\\preferences};

\node[draw, rounded corners=2pt, fit=(roster)(msgs)(policy)(desc)(profile),
      inner sep=4pt, label={[font=\scriptsize]above:Signed group state}] {};
\node[draw, rounded corners=2pt, fit=(sessions)(cursors)(search)(dir)(push),
      inner sep=4pt, label={[font=\scriptsize]below:ESP-local projection}] {};

\draw[arrow] (roster) -- (msgs);
\draw[arrow] (policy) -- (roster);
\draw[arrow] (desc) -- (roster);
\draw[arrow] (profile) -- (roster);
\draw[arrow] (msgs) -- (search);
\draw[arrow] (roster) -- (sessions);
\draw[arrow] (desc) -- (dir);
\draw[arrow, dashed] (search) -- node[right, font=\scriptsize] {cannot forge} (msgs);
\end{tikzpicture}
...
\node[joinstep] (verify) at (0, 1.6) {verify signed\\invite/descriptor};
\node[joinstep] (trust) at (3.7, 1.6) {establish or reuse\\Pilot trust};
\node[joinstep] (roster) at (7.4, 1.6) {request signed\\roster};
\node[joinstep] (apply) at (7.4, 0.1) {apply roster\\and policy checks};
\node[joinstep] (snap) at (3.7, 0.1) {pull profile and\\transport snapshots};
\node[joinstep] (sync) at (0, 0.1) {reconcile missing\\message history};
...
\draw[arrow, dashed] (sync.west) -- ++(-0.7,0) |- node[left, font=\scriptsize]
  {caught up} (verify.west);
\end{tikzpicture}
...
has both \texttt{doctor=0} and \texttt{peers=0}.}
\label{tab:evaluation-health}
\begin{tabular}{lrrll}
\toprule
Participant & Healthy rows & Rate & Longest healthy & Longest unhealthy \\
\midrule
\texttt{local-vps} & 243 / 245 & 99.2\% & -- & -- \\
\texttt{hermes-container} & 244 / 245 & 99.6\% & -- & -- \\
\texttt{deimos-openclaw-container} & 244 / 245 & 99.6\% & -- & -- \\
...
The run measured operational health and final state more directly than
latency. It saved timestamped snapshots and transcripts, but this paper does
not report a propagation-latency distribution, per-message recovery latency,
or root-by-root convergence timeline. Future runs should instrument publish,
first receipt, query visibility, Merkle-root agreement, and search/context
correctness as first-class metrics rather than reconstructing them from logs.
...
benchmark release. A future artifact-badged release should publish a sanitized
package, the scripts that generated the analysis tables, exact runtime
versions, and a replayable subset of transcripts or synthetic equivalents.
This paper keeps the private run evidence available locally and uses the final
checksums to make accidental drift detectable.
