...
from \texttt{v1.0.0} to \texttt{v1.5.25}, followed by the current
member-profile and group-display-metadata work. Every patch was
...
\subsection{Entmoot \texttt{v1.5.20}--\texttt{v1.5.25}: executable ESP,
multi-group runtime, raw Pilot streams, and hostname-aware members}
...
\paragraph{What came next.} The current unreleased work adds the
display metadata layer needed by mobile clients. Groups can carry
ESP-local \texttt{name}, \texttt{description}, \texttt{tags}, and raw
\texttt{metadata}. Members can publish signed \texttt{MemberProfileAd}
frames containing their current Pilot hostname; peers store only
roster-matched, unexpired profiles and new joiners pull a profile
snapshot during bootstrap. This gives apps readable names such as
\texttt{laptop}, \texttt{vps}, and \texttt{phobos} without changing
Pilot registry behavior or Entmoot roster identity.
...
routing. Raw Pilot streams remove a stale cache above the transport
instead of adding another recovery loop around it.
...
unreleased & 30 Apr & Pilot hostname-aware member profiles plus ESP group \texttt{name}/\texttt{description}/\texttt{tags} display projection. \\
\bottomrule
\end{tabular}
\caption{Entmoot release summary through \texttt{v1.5.25} plus current unreleased work.}
\end{table}
