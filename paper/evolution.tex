...
\begin{abstract}
\noindent
Between 17 April and 3 May 2026 we shipped a dense sequence of
patch releases across two layered systems: the Pilot fork moved from
\texttt{v1.7.2-jf.1} to \texttt{v1.9.0-jf.15.24}, and Entmoot moved
from \texttt{v1.0.0} to \texttt{v1.5.35}. Every patch was
...
\begin{itemize}[leftmargin=1.2em]
    ...
    Later operational releases add \texttt{entmootd env} plus a
    \texttt{/data/.entmoot/entmoot} wrapper so container agents can
    detect socket-namespace mistakes and consistently use
    \texttt{/data/.pilot/pilot.sock}.
...
\subsection{Pilot \texttt{jf.15.8}--\texttt{jf.15.24}: the stream
...
\subsection{Entmoot \texttt{v1.5.11}--\texttt{v1.5.19}: mobile service
peer primitives and release observability}

\paragraph{What broke.} After the transport and reconcile paths were
green, the next question was not another packet bug. It was product
shape: an iOS app cannot be expected to run a full always-on
\texttt{pilot-daemon} plus \texttt{entmootd join} process in the
background. We needed the infrastructure for an always-on service peer
that can help intermittent clients catch up and publish without taking
custody of their signing keys.

\paragraph{What it actually was.} The existing group protocol already
stored and reconciled messages, but it lacked first-class service-peer
interfaces. There was no durable per-client mailbox cursor, no HTTP
sync surface, no local device registry, no request-signature helper,
and no explicit release metadata command to inventory which binary a
peer was actually running.

\paragraph{How we fixed it.} Entmoot \texttt{v1.5.11} introduced the
signer abstraction, message-ingest events, and mobile service-peer
building blocks. \texttt{v1.5.12} moved publish success fully behind
the durable local accept boundary so trust IPC stalls do not make a
stored message look failed. \texttt{v1.5.13} added durable SQLite
mailbox cursors. \texttt{v1.5.14} exposed those cursors through local
\texttt{mailbox} CLI commands. \texttt{v1.5.15} added the loopback-first
ESP HTTP bridge. \texttt{v1.5.16} added phone-signed publish forwarding.
\texttt{v1.5.17} added device-authenticated HTTP requests and a local
device registry. \texttt{v1.5.18} added operator CLI helpers for device
onboarding and request signing. \texttt{v1.5.19} added
\texttt{entmootd version} so release verification no longer depends on
guessing which binary a service manager launched.

\paragraph{Why the fix is right.} The phone should hold the phone's
identity; the service peer should hold infrastructure. That mirrors the
operational lesson from Lightning-style mobile systems without copying
their protocol: keep the always-on node available, keep authoring keys
off the server when desired, and make sync explicit and durable. The
ESP path does not change Entmoot's consensus state; it packages the
existing store, roster validation, and gossip fanout into interfaces an
intermittent client can safely use.

\subsection{Entmoot \texttt{v1.5.20}--\texttt{v1.5.35}: executable ESP,
multi-group runtime, raw Pilot streams, onboarding, and diagnostics}

\paragraph{What broke.} Once the ESP surface became usable by an iOS
client, two practical gaps appeared. First, mobile operations beyond
message publish still required operator-side manual steps: group
creation, group metadata updates, invite creation, and invite
acceptance were queued as sign requests but not executed end to end.
Second, live propagation still had stale-session failure modes because
Entmoot kept a cached yamux session over Pilot streams even though
Pilot itself already provides stream semantics.

\paragraph{What it actually was.} The ESP needed a clear split between
phone authorization and service-peer execution. The phone signs a
canonical operation payload; the ESP verifies it and then asks the
running \texttt{join} process to perform the operation. In parallel,
the transport layer needed to stop layering a long-lived application
multiplexer over a lower-layer virtual stream. Each gossip or
reconcile interaction is now a short-lived Pilot stream with explicit
per-peer dial budgeting and write deadlines.

\paragraph{How we fixed it.} \texttt{v1.5.20} added the expanded ESP
HTTP surface: session/status, groups, members, notification state, and
durable sign requests. \texttt{v1.5.21} made group and invite
operations executable and added a bounded \texttt{/history} endpoint
for initial mobile timeline rendering without moving mailbox cursors.
\texttt{v1.5.22} removed the yamux session cache above Pilot and moved
to one Pilot stream per interaction. \texttt{v1.5.23} and
\texttt{v1.5.24} restored tunnel-up callbacks and made fanout budgets
Pilot-aware so TURN recovery and NAT refresh get enough time without
blocking every message on a fixed long timeout. \texttt{v1.5.25}
forwarded those Pilot transport capabilities through the multi-group
mux so every active group uses the same retry and stale-stream policy.

\paragraph{What came next.} The following releases turned the mobile
and onboarding surfaces into production paths. Groups can carry
ESP-local \texttt{name}, \texttt{description}, \texttt{tags}, and raw
\texttt{metadata}. Members publish signed \texttt{MemberProfileAd}
frames containing their current Pilot hostname; peers store only
roster-matched, unexpired profiles and new joiners pull bounded
profile snapshots during bootstrap. Open invites add an issuer URL,
expiry, and max-use count; redeemers prove Pilot key possession with a
domain-separated challenge before the issuer returns a normal signed
invite. \texttt{join} now auto-redeems those links, starts from
persisted state via \texttt{serve}, sends bounded Pilot onboarding
handshakes to roster/bootstrap/founder candidates, and emits a compact
health summary with the next \texttt{doctor} command. Existing members
auto-approve only pending Pilot handshakes from current roster members
whose Pilot key matches the known identity. \texttt{doctor} and
\texttt{peers} expose the state operators were previously inferring by
hand: roster membership, profile visibility, transport-ad freshness,
Pilot trust, route probes, diagnoses, and suggested repairs.

\paragraph{Why the fix is right.} The pattern is the same in both
places: keep authoritative protocol state small and signed, and put
service/UI convenience in explicitly local or signed cache layers. ESP
metadata does not rewrite rosters. Member hostnames do not decide
routing. Open invites still resolve to signed roster invites. Raw
Pilot streams remove a stale cache above the transport instead of
adding another recovery loop around it. Diagnostics make the
``known peer'' versus ``routable peer'' distinction explicit.
...
v1.9.0-jf.15.22--jf.15.24 & 30 Apr--3 May & Tracked stream-send acknowledgements, lookup/sign-challenge IPC, domain-separated application challenge signing, and pending-handshake notifications for group auto-approval. \\
\bottomrule
\end{tabular}
\caption{Pilot fork release summary through \texttt{v1.9.0-jf.15.24}.}
\end{table}
...
v1.5.11--v1.5.12 & 28 Apr & Mobile service-peer primitives, signer abstraction, ingest events, and publish success after durable local accept. \\
v1.5.13--v1.5.16 & 27--28 Apr & Durable mailbox cursors, local mailbox CLI, ESP HTTP bridge, and phone-signed publish forwarding. \\
v1.5.17--v1.5.19 & 28 Apr & ESP device-auth registry, request-signing helper, and \texttt{entmootd version} release metadata. \\
v1.5.20--v1.5.21 & 29 Apr & Expanded ESP mobile API, executable group/invite sign requests, ESP-local group metadata, and bounded latest-history reads. \\
v1.5.22--v1.5.25 & 29 Apr & Raw Pilot streams per interaction, stale stream invalidation, Pilot-aware fanout budgets, tunnel-up callback repair, and multi-group transport capability forwarding. \\
v1.5.26--v1.5.30 & 30 Apr--1 May & Remembered \texttt{serve}, member-profile convergence repair, strict Pilot stream send acknowledgements, large-response timeouts, and dedicated Pilot IPC drivers. \\
v1.5.31--v1.5.33 & 1--2 May & ESP admin controls, targeted/open invites, member removal, live roster fanout, Pilot proof-of-possession for open invites, and retry-safe redemption. \\
v1.5.34--v1.5.35 & 2--3 May & Auto-redeem open invites in \texttt{join}, automatic Pilot onboarding handshakes and auto-approval, \texttt{doctor}/\texttt{peers} diagnostics, repair suggestions, and join health summaries. \\
\bottomrule
\end{tabular}
\caption{Entmoot release summary through \texttt{v1.5.35}.}
\end{table}
